...
import { workoutToDefinition, toJSON } from "@/lib/export/structured-workout-export";
...
export async function GET(
  ...
  const allowed =
    workout.week.plan.athlete.userId === uid ||
    workout.week.plan.coachId === uid;
  if (!allowed) return NextResponse.json({ error: "Forbidden" }, { status: 403 });

  const definition = workoutToDefinition({ ...workout, workoutType: workout.type });
  const json = toJSON(definition);

  return new Response(json, {
    headers: {
      "Content-Type": "application/json",
      "Content-Disposition": `attachment; filename="treino-${id}.json"`,
    },
  });
}
