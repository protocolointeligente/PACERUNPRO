...
// GET — own affiliate info + referral stats
...
export async function POST(req: NextRequest) {
  ...
  if (existing) return NextResponse.json({ error: "Você já possui um programa de afiliado" }, { status: 409 });

  const codeConflict = await prisma.marketplaceAffiliate.findUnique({
    where: { code },
    select: { id: true },
  });
  if (codeConflict) return NextResponse.json({ error: "Este código já está em uso. Escolha outro." }, { status: 409 });

  const affiliate = await prisma.marketplaceAffiliate.create({
    data: { userId: session.user.id, code, commissionPct: 0.10 },
