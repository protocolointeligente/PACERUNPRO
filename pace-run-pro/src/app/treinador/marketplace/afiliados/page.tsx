...
export default function AfiliadosPage() {
  ...
  const load = () => {
    fetch("/api/coach/marketplace/affiliates")
  ...
  async function handleCreate() {
    setCreating(true);
    setError(null);
    const res = await fetch("/api/coach/marketplace/affiliates", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ code: code.trim(), commissionPct: 0.10 }),
    });
  ...
  function copyLink() {
    ...
    navigator.clipboard.writeText(`${appUrl}/marketplace?ref=${affiliate.code}`);
    setCopied(true);
    ...
    <div className="mx-auto max-w-3xl space-y-8 px-4 py-8 sm:px-6">
      <div>
        ...
        <p className="text-sm text-text-muted mt-0.5">
          Compartilhe o marketplace e ganhe comissão sobre cada venda indicada pelo seu link exclusivo.
        </p>
      </div>
      ...
      {!affiliate ? (
        <Card>
          <CardContent className="p-6 space-y-4">
            <h2 className="font-display text-base font-semibold text-text">Criar meu código de afiliado</h2>
            <p className="text-sm text-text-muted">
              Escolha um código único (ex.: MARCORUNS). Você ganhará <strong className="text-text">10% de comissão</strong> sobre cada venda realizada via seu link.
            </p>
        ...
        <>
          ...
          <Card className="border-primary/20">
            <CardContent className="p-5 space-y-3">
              <div className="flex items-center justify-between">
                <h2 className="font-display text-base font-semibold text-text">Seu link de afiliado</h2>
              ...
              <div className="flex gap-2 items-center rounded-xl border border-border bg-card-hover/40 px-4 py-3">
                <code className="flex-1 text-sm text-text-muted truncate">
              ...
              <p className="text-xs text-text-muted">
                ...
                {" · "}Comissão: <span className="font-bold text-success">{(affiliate.commissionPct * 100).toFixed(0)}%</span> por venda
              </p>
            </CardContent>
          </Card>
                ...
                <div className="space-y-2">
                  {affiliate.recentReferrals.map((r, i) => (
                    <div key={i} className="flex items-center justify-between gap-3 rounded-xl border border-border px-4 py-3">
                      <div>
                        <p className="text-xs text-text-muted">{new Date(r.createdAt).toLocaleDateString("pt-BR", { day: "2-digit", month: "short", year: "numeric" })}</p>
          ...
          <div className="rounded-xl border border-border/40 bg-card-hover/30 px-4 py-3 text-xs text-text-muted">
            Os ganhos são confirmados após a conclusão do pagamento e repassados junto com suas comissões mensais no dia 10.
          </div>
        </>
      )}
      ...
      <Card>
        <CardContent className="p-5 space-y-4">
          ...
          <div className="space-y-3">
            {[
              { n: "1", text: "Compartilhe seu link exclusivo com atletas, nas redes sociais ou por WhatsApp." },
              { n: "2", text: "Quando alguém acessar o marketplace pelo seu link e realizar uma compra, você ganha comissão automaticamente." },
              { n: "3", text: "Os ganhos são consolidados mensalmente e repassados junto com suas comissões de venda." },
            ].map(({ n, text }) => (
